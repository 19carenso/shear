...
class Handler():
    ...
    def load_var(self, casestudy, var_id, i_t, z_idx = None, sel_dict = None): 
        ...
        if var_id in new_var_names:
            ...
            da_new_var = load_func(casestudy, i_t) 
            return da_new_var
    ...
    def load_prec(self, casestudy, i_t):
        """
        ...
        if i_t in self.settings["prec_i_t_bug_precac"]:
            previous_precac = self.load_var(casestudy, 'Precac', i_t-2)[0]
        else : 
            previous_precac = self.load_var(casestudy, 'Precac', i_t-1)[0]

        current_precac = self.load_var(casestudy, 'Precac', i_t)[0]
