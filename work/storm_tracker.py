...
class StormTracker():
    ...
    def load_storms_tracking(self, overwrite):
        ...
        if os.path.exists(file_storms) and not overwrite:
            ...
            filtered_storms = []
            lat_min, lat_max = self. r["BOX"][0], self.settings["BOX"][1]
            for storm in storms:
    ...
    def get_frame_data(self, ft, iDCS):
        ...
        start = np.max([
            0,
            int(storm.INT_UTC_timeInit.values/self.settings["NATIVE_TIMESTEP"])-self.i_t_start
            ])
        end = np.min([
            1+int(storm.INT_UTC_timeEnd.values/self.settings["NATIVE_TIMESTEP"])-self.i_t_start, 
            self.i_t_end
             ])
