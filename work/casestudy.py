...
class CaseStudy():
    ...
    def _set_variables(self, overwrite):
        ...
        if overwrite or not os.path.exists(json_path):
            ...
            if self.model ==  'DYAMOND_SAM_post_20_days' or self.model == 'DYAMOND_SAM' or self.model == "SAM_4km_30min_30d" or self.model == "SAM3d":
    ...
    def get_valid_3d(self):
        valid_3d = []
        for key in self.days_i_t_per_var_id["V"].keys(): #V because it has fewer 3d fields than other but stay flexible here
            for vt in self.days_i_t_per_var_id["V"][key]:
                valid_3d.append(vt)
        return valid_3d
